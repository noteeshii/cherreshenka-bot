...
import YandexMusicClient from '#extensions/music/providers/Yandex.ts';
...
test('preview не выдаётся за полный трек', async () => {
  ...
  await assert.rejects(
    new YandexMusicClient({ ...testConfig().music, yandexMusicToken: '' }, request).resolve(
      url,
...
test('HTTP 404 и отказ авторизации дают понятную ошибку', async () => {
  for (const [status, message] of [
    ...
    await assert.rejects(
      new YandexMusicClient({ ...testConfig().music, yandexMusicToken: 'secret' }, request).resolve(
        url,
        new AbortController().signal,
      ),
      message,
...
test('hex timestamp Яндекса сохраняется вместе с ведущими нулями', async () => {
  ...
  const sign = createHash('md5').update('XGRlBW9FXlekgbPrRHuSiAaudio.mp3salt').digest('hex');
  assert.equal(track.url, `https://cdn.yandex.net/get-mp3/${sign}/${timestamp}/audio.mp3`);
});
